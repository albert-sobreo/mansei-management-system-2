...
class ADVapprovalAPI(APIView):
    def post(self, request):
        ...
        j = JournalAPI(request, adv.code, adv.issuer, adv.datatimeApproved, "Advancement Journal")

        request.user.branch.journal.add(j)

        # jeAPI(request, j, 'Credit', dChildAccount.pettyCash, adv.amount)
        j.addJE('Credit', dChildAccount.pettyCash, adv.amount)
        try:
            # jeAPI(request, j, 'Debit', adv.requestor.employeeAccounts.get(name__regex=r"[Ad]dvance"), adv.amount)
            j.addJE('Debit', adv.requestor.employeeAccounts.get(name__regex=r"[Ad]dvance"), adv.amount)
        except:
            ...
            j.addJE('Debit',emploAcc, adv.amount)
        j.save()
...
class ReimbursementProcess(APIView):
    def post(self, request):
        ...
        # j.save()
        j = JournalAPI(request, lqd.code, lqd.createdBy, datetime.datetime.now(), "Reimbursement Journal")
        # jeAPI(request, j, 'Credit', dChildAccount.pettyCash, lqd.payable)
        j.addJE('Credit', dChildAccount.pettyCash, lqd.payable)
        # jeAPI(request, j, 'Debit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Pp]ayable"), lqd.payable)
        j.addJE('Debit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Pp]ayable"), lqd.payable)
        #### B0SS ####
        j.save()
        """END OF JOURNAL"""
...
class LiquidationApprovalAPI(APIView):
    def post(self, request):
        ...
        # j.journalDate = datetime.date.today()
        # j.save()
        j = JournalAPI(request, lqd.code, lqd.createdBy, datetime.datetime.now(), "Liquidation Journal")
        if lqd.advancement:
            ##### PERFECT LIQUIDATION #####
            ...
            if lqd.change == 0 and lqd.payable == 0:
                ...
                for l in lqd.liquidationentries.all():
                    ...
                    j.addJE('Debit', l.expense, l.amount)
                # jeAPI(request, j, 'Credit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Ad]dvance"), lqd.totalAmount)
                j.addJE('Credit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Ad]dvance"), lqd.totalAmount)
            ...
            ##### LIQUIDATION W/ CHANGE #####
                ...
                j.addJE('Debit', dChildAccount.pettyCash, lqd.change)
                # jeAPI(request, j, 'Credit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Ad]dvance"), lqd.totalAmount + lqd.change)
                j.addJE('Credit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Ad]dvance"), lqd.totalAmount + lqd.change)
                lqd.advancement.balance = 0
                lqd.advancement.closed = True
                lqd.advancement.save()
                lqd.save()
            ...
            ##### LIQUIDATION W/ PAYABLES #####
                ...
                j.addJE('Credit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Ad]dvance"), lqd.advancement.balance)
                try:
                    # jeAPI(request, j, 'Credit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Pp]ayable"), lqd.payable)
                    j.addJE('Credit', lqd.createdBy.employeeAccounts.get(name__regex=r"[Pp]ayable"), lqd.payable)
                except:
                    emploAcc = AccountChild()
                    try:
                    ...
                    j.addJE('Credit', emploAcc, lqd.payable)
                lqd.advancement.balance = 0
                lqd.advancement.closed = True
                lqd.advancement.save()
            ...
            j.addJE('Credit', dChildAccount.pettyCash, lqd.totalAmount)
...
class ExportsApprovalAPI(APIView):
    def post(self, request):
        ...
        # j.save()
        # request.user.branch.journal.add(j)
        j = JournalAPI(request, ex.code, ex.creadtedBy, ex.dateSold, "Export Journal")
        ...
        if totalFees != 0.0:
            ...
            j.addJE('Credit', dChildAccount.otherIncome, (totalFees*ex.forex))
        ...
        for item in ex.exportitemsmerch.all():
            ...
            j.addJE('Credit', item.merchInventory.childAccountSales, (item.totalCost*ex.forex)-((ex.discountPeso/ex.exportitemsmerch.all().count())*ex.forex))

        # jeAPI(request, j, 'Debit', ex.party.accountChild.get(name__regex=r"[Rr]eceivable"), (ex.amountTotal*ex.forex))
        j.addJE('Debit', ex.party.accountChild.get(name__regex=r"[Rr]eceivable"), (ex.amountTotal*ex.forex))
